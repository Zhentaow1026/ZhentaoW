...
- Run:
```bash
sbatch scripts/batlow/slurm_batlow_make_sync_2pop1.sbatch
```
...
- Run:
```bash
sbatch scripts/07B_Batlow_allelefrequency_diff+FST_per_SNP+FST_window+Fisher_exact_test.sbatch
```
...
- Run:
```bash
python scripts/06A_plot_coverage_batlow.py
```
- Output:
...
- Run:
```bash
sbatch scripts/young/slurm_young_make_sync_2pop1.sbatch
```
